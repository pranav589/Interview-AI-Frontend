...
import { ArrowLeft, ThumbsUp, AlertCircle, Lightbulb, Loader2 } from 'lucide-react';
import TranscriptViewer from './transcript-viewer';
import FeedbackScores from './feedback-scores';
...
export default function InterviewDetailPage({ interviewId }: InterviewDetailPageProps) {
  const { data: interview, isLoading, isError } = useInterviewDetails(interviewId);
        ...
        <div className="flex flex-col items-center justify-center px-4 py-20">
          <Card className="max-w-md w-full">
            <CardContent className="pt-6 text-center">
              ...
              <p className="text-muted-foreground mb-4">Interview not found or failed to load</p>
              <Link href="/dashboard">
  ...
  const feedback = {
    ...
    areasForImprovement: feedbackData?.areasForImprovement || ["Working on insights..."],
    suggestions: feedbackData?.suggestions || ["Hang tight!"],
    feedbackSummary: feedbackData?.feedbackSummary || "Detailed feedback is being processed.",
        ...
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <p className="text-muted-foreground">
              {interview.company || "General"} • {interview.jobTitle || "Software Engineer"}
            </p>
          ...
          >
            <div className="relative group">
              <svg viewBox="0 0 120 120" className="w-56 h-56 transition-transform duration-500 group-hover:scale-105 drop-shadow-2xl">
                <defs>
                ...
                <motion.circle cx="60" cy="60" r="52" fill="none" 
                  ...
                  animate={{ strokeDashoffset: 2 * Math.PI * 52 * (1 - feedback.overallScore / 100) }}
                  transition={{ duration: 1.5, ease: "circOut", delay: 0.5 }}
                  transform="rotate(-90 60 60)"
                  style={{ filter: 'drop-shadow(0 0 8px oklch(0.6 0.25 270 / 0.3))' }}
                />
              ...
              <div className="absolute -top-4 -right-4 bg-primary text-primary-foreground px-3 py-1 rounded-full text-xs font-bold shadow-lg animate-bounce duration-[3000ms]">
                {feedback.overallScore >= 80 ? "EXCELLENT" : feedback.overallScore >= 60 ? "GOOD" : "IMPROVING"}
              </div>
            </div>
                  ...
                  <Card key={index} className="overflow-hidden">
                    <CardHeader className="bg-muted/30">
                      <div className="flex items-center justify-between gap-4">
                        <CardTitle className="text-lg font-semibold flex-1">Q{index + 1}: {q.question}</CardTitle>
                        <div className={`text-xl font-bold px-3 py-1 rounded-full ${q.score >= 70 ? 'bg-green-100 text-green-700' : q.score >= 40 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>
                          {q.score}/100
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="pt-6 space-y-4">
                      <div>
                        ...
                        <p className="text-sm bg-muted/50 p-3 rounded-lg border italic">"{q.userAnswer}"</p>
                      </div>
                      <div>
                        <h4 className="font-semibold text-sm text-muted-foreground mb-1">Feedback</h4>
          ...
          <motion.div
            ...
            className="mb-8"
          >
            <TranscriptViewer transcript={interview.transcriptions.map((t, i) => ({
              id: `t-${i}`,
          ...
          >
            <Link href="/dashboard" className="w-full sm:w-auto">
              <Button variant="outline" size="lg" className="w-full sm:w-48 h-12">
                Back to Dashboard
              </Button>
            </Link>
            <Link href={`/interview-setup?type=${interview.interviewType}&difficulty=${interview.difficultyLevel}`} className="w-full sm:w-auto">
              <Button size="lg" className="w-full sm:w-48 h-12 shadow-md shadow-primary/20">
                Practice Again
              </Button>
            </Link>
          </motion.div>
